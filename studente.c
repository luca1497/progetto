...
#include "common.h"
char *up(char *s){
    char *p = strdup(s);  // Duplicate the input string to a new buffer
    char *start = p;  // Save the pointer to the start of the buffer
    while (*p != '\0') {
...
int main(int argc, char **argv)
{
    ...
    scanf("%s",richiesta->matricola);
    printf("Matricola: %s", richiesta->matricola);
